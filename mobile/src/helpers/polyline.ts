...
export const buildArrowMarkers = (points?: LatLng[], approxIntervalMeters: number = 500) => {
    if (!points || points.length < 2) return [] as { coordinate: LatLng; bearing: number }[];
    ...
    const markerCount = Math.min(MAX_MARKERS, Math.max(1, Math.round(totalDistance / approxIntervalMeters)));
    ...
    for (let i = 0; i < points.length - 1 && markers.length < markerCount; i++) {
        ...
        while (nextThreshold <= accumulated + segDist && markers.length < markerCount) {
...
const nycPolylineBase = [
  { latitude: 40.74015, longitude: -73.99690 },
  { latitude: 40.74015, longitude: -73.99590 },
  { latitude: 40.74195, longitude: -73.99590 },
  { latitude: 40.74195, longitude: -73.99980 },
  { latitude: 40.74015, longitude: -73.99980 },
];
